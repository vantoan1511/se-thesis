...
public interface ArticleRepo extends JpaRepository<Article, Long> {
    ...
    @Query(value = """
            SELECT DISTINCT a FROM Article a LEFT JOIN FETCH a.category c \
            WHERE a.featured=:featured AND a.state.code=:stateCode ORDER BY a.publishedAt DESC
            """,
            countQuery = """
                    SELECT COUNT( DISTINCT a) FROM Article a LEFT JOIN a.category c \
                    WHERE a.featured=:featured AND a.state.code=:stateCode
                    """)
    ...
    @Query(value = """
            SELECT DISTINCT a FROM Article a LEFT JOIN FETCH a.category \
            WHERE a.state.code=:stateCode ORDER BY a.publishedAt DESC
            """,
    ...
    Page<Article> findAllByFeaturedAndStateCode(boolean featured, String stateCode, Pageable pageable);
}
